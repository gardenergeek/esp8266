...
{	
	...
	{
		m_lastReadTs = 0;
		m_state = DHTERROR_NOTCONFIGURED;
		m_dataBuffer = new unsigned char[DATABITS];
	}
	Dht22::~Dht22()
	{
	...
	bool Dht22::configure()
	{
		...
		// Attach irq handler
		m_gpio.attachInterruptHandler(__isrFunc,this,core::GPIO_PIN_INTR_ANYEDGE);
	...
	bool Dht22::read(int *humidity,int *temperature)
	{
		...
		if(m_state == DHTERROR_NOTCONFIGURED)
			return false;
	...
	const char *Dht22::getLastResultMessage()
	{
		switch(m_state)
		{
			...
			case DHTERROR_NOTCONFIGURED :
			return "call Dht22.configure() before use";	
	...
	IRAM_ATTR void __isrFunc(void *instance)
	{
		((Dht22 *)instance)->isr();
	}
